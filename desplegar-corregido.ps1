...
if ($LASTEXITCODE -eq 0) {
    ...
    gcloud sql instances create cmms-db --database-version=POSTGRES_15 --tier=db-f1-micro --region=us-central1 --root-password=$dbPassword --backup-start-time=03:00 --enable-bin-log --retained-backups-count=7 --retained-transaction-log-days=7 --project=argon-edge-478500-i8
...
'@
    ...
    gcloud run deploy cmms-backend --source . --region=us-central1 --platform=managed --allow-unauthenticated --set-env-vars="DJANGO_SETTINGS_MODULE=config.settings.production,DB_NAME=cmms_prod,DB_USER=cmms_user,DB_PASSWORD=$dbPassword,DB_HOST=/cloudsql/$connectionName" --add-cloudsql-instances=$connectionName --memory=1Gi --cpu=1 --min-instances=0 --max-instances=10 --project=argon-edge-478500-i8
